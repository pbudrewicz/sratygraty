...
#	next;	
    ...
    print "\n" if $SKIPPED;
    $SKIPPED = 0;
    printf "Time: %s, units left: $UFMT, time left: %s, speed: %s(%s|%s)                 \r", 
    &timestr($TIME), $CNT, 
...
#    &speedstr($LOCALMEAN ? $LOCALMEAN[1]-$LOCALMEAN[0]:$SCNT-$CNT-$LCNT,  $LOCALMEAN ? ($LOCALTIME[0] - $LOCALTIME[1] ) : ($TIME - $LTIME)), 
    &speedstr($TIME == $LTIME ? 0 : (($SCNT-$CNT)*($TIME-$LTIME) - ($SCNT-$CNT-$LCNT)*$TIME) / ($TIME * ($TIME - $LTIME)), 1), 
...
printf "Done. Total time: %s, Speed: %s                              \n", &timestr($TIME), &speedstr( $SCNT, $TIME );
